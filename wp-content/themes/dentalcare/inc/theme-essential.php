...
function dentalcare_comment( $comment, $args, $depth )
{
    ...
    <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
        <p><?php esc_html__( 'Pingback:', 'dentalcare' ); comment_author_link(); edit_comment_link( esc_html__( '(Edit)', 'dentalcare' ), '<span class="edit-link">', '</span>' ); ?>
		</p>
    <?php
            break;
        default :
...
{	
	...
	if ( is_home() || is_category() || is_search() || is_tag() || is_tax() )
	{
		...
		if(!empty($dentalcare_option['title_background']['background-image']))
		{
		...
		{	
			if(!is_single())
			{
				$innerheader = $dentalcare_option['blog_background']['background-image'];
			}
		}
		if(isset($dentalcare_option['tilebar_layout'] ) && $dentalcare_option['tilebar_layout'] != 1)
		{
			$innerheader = '';
		} 
		...
		{   
			if(isset($dentalcare_option['tilebar_layout_main']) && $dentalcare_option['tilebar_layout_main'] == '1')
			{ 
				if($dentalcare_option['tilebar_layout'] || $dentalcare_option['title_switch'] == '1')
				{ ?>
					<div class="tt-heading">
						<div class="tt-heading-cell" style="background-image:url(<?php echo esc_url($innerheader); ?> );
						<?php echo esc_attr($titlePaddingTop);  echo esc_attr($titlePaddingBottom); ?>">
							<div class="container">
								<?php if($HidePageTitle != 'yes') 
								{
									if(isset($dentalcare_option['title_switch']) && $dentalcare_option['title_switch'] == '1')
									{
										if( dentalcare_page_title( false) ): 
											if($PageHeaderTitle != '') 
											{ ?>
									  ...
									  <?php } 
											elseif($dentalcare_post_type == 'post') 
											{ 
												if(!empty($dentalcare_option['blog_title']))
												{ ?>
